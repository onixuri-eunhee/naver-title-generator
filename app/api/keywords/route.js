 ...
 */
import {
  getRedis, resolveAdmin, extractToken, resolveSessionEmail, getClientIp, isCreditsActive,
...
const NO_CACHE_HEADERS = {
  ...
  'Pragma': 'no-cache',
  'X-Keywords-Version': DEBUG_VERSION,
};
...
export async function POST(request) {
  ...
  if (!isAdmin && !email) {
    return jsonResponse(request, { error: '로그인이 필요합니다.' }, { status: 401, headers: NO_CACHE_HEADERS });
  }
  ...
  const { field, role, target, questions, userSeeds } = body;
  ...
  if (!field || !role || !target) {
    ...
    return jsonResponse(request, { error: '내 분야, 나는, 타겟 독자를 모두 입력해주세요.' }, { status: 400, headers: NO_CACHE_HEADERS });
  }
  ...
  try {
    ...
    return jsonResponse(request, { error: '키워드 분석 중 오류가 발생했습니다. 잠시 후 다시 시도해주세요.' }, { status: 500, headers: NO_CACHE_HEADERS });
  }
}
