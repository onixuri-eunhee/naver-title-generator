...
export async function GET(request) {
  try {
    ...
    if (!email) {
      return jsonResponse(request, { remaining: 0, limit: FREE_DAILY_LIMIT, loginRequired: true, creditsActive: isCreditsActive() });
    }
    if (isCreditsActive()) {
      const credits = await getUserCredits(email);
      return jsonResponse(request, { remaining: credits, creditCost: THREAD_CREDIT_COST, creditsActive: true });
    }
    ...
    return jsonResponse(request, { remaining: 0, limit: FREE_DAILY_LIMIT, creditsActive: isCreditsActive() });
  }
}
...
export async function POST(request) {
  try {
    ...
    if (!topic) {
      return jsonResponse(request, { error: '주제/소재를 입력해주세요.' }, { status: 400 });
    }
