...
첫 줄부터 곧바로 후킹문구1을 출력한다. 분석/메모/구분선/설명/번호 어떤 것도 앞에 붙이지 않는다.

✅ 올바른 출력 (첫 줄부터 곧바로 후킹문구):
이거 모르고 가면 웨이팅 1시간 더 해요
단골만 아는 숨은 자리가 있어요
강사 경력 15년인데 AI 안 쓰는 분 봤어요
... (총 15줄)

❌ 잘못된 출력 (절대 이렇게 하지 마라):
온라인 강사(B2B) 타겟, AI 업무 자동화 키워드 분석 완료.
Tier 1 공식(패턴 인터럽트, 손실회피, 호기심폭발) 비중 높임.
강의 수익화, 학생 관리, 강의 운영 효율화 중심으로 생성.
---
이거 모르고 가면 웨이팅 1시간 더 해요
...

❌ 또 잘못된 출력 (번호/마크다운/이모지 도배):
1. 이거 모르고 가면 웨이팅 1시간 더 해요
**2. 단골만 아는 숨은 자리가 있어요**
🔥3. 강사 경력 15년인데 AI 안 쓰는 분 봤어요

분석은 머릿속으로만 하고, 결과 15줄만 깨끗하게 출력한다.`;
...
export async function POST(request) {
  ...
  try {
    ...
    let { industry, keyword } = body;
    industry = (industry || '').substring(0, 50);
    keyword = (keyword || '').substring(0, 100);

    if (!industry) {
      return jsonResponse(request, { error: '업종을 입력해주세요.' }, { status: 400 });
    }
    if (!keyword) {
...
14가지 심리 공식을 골고루 활용하되 Tier 1 비율을 높이고, 업종 특성에 맞는 구체적인 표현을 사용해주세요.`;
    ...
    if (!response.ok) {
      ...
      return jsonResponse(request, { error: '후킹문구 생성 중 오류가 발생했습니다.' }, { status: 500 });
    }
