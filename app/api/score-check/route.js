...
export async function POST(request) {
  ...
  const { action, text, parsed } = body;

  if (action === 'score') {
    if (!text || typeof text !== 'string') {
      return jsonResponse(request, { error: 'text 필드가 필요합니다.' }, { status: 400 });
    }
    ...
    return jsonResponse(request, { results, totalScore, maxScore: 90 });
  }

  if (action === 'replace') {
    if (!parsed || typeof parsed !== 'object') {
      return jsonResponse(request, { error: 'parsed 객체가 필요합니다.' }, { status: 400 });
    }

    const replaced = replaceAIVocabulary({ ...parsed });
    return jsonResponse(request, { parsed: replaced });
  }

  return jsonResponse(request, { error: 'action은 "score" 또는 "replace"여야 합니다.' }, { status: 400 });
}
