...
첫문장 공식: 솔직 고백 오프닝(나도 ~했어) 또는 타겟 호명(~하는 사람, 나야 나)
...
첫문장 공식: [상식/통념]이라고? 거짓말이다 또는 [행동] 하지 마. 그게 망하는 길이다
...
[강제 규칙 #0 — 사용자 입력이 최우선]
사용자가 입력한 "업종", "타겟", "소재", "메모"가 글의 핵심 주제다.
이 4가지를 무시한 글은 아무리 잘 써도 실패다.
모든 글은 사용자의 소재와 메모를 직접적으로 다뤄야 한다.
후킹 공식이나 형식 규칙보다 사용자 입력 반영이 우선이다.
...
위 4가지가 글의 핵심이다. 업종(${industry || '무관'})과 타겟(${target || '일반'})에 맞는 구체적 상황과 숫자를 사용하고, 소재(${topic})를 직접 다뤄라. 메모가 있으면 그 방향대로 써라.

유형: ${type || '정보형'} (${typeGuide[type] || typeGuide['정보형']})
말투: ${toneGuide[tone] || toneGuide['친구체']} — 이 말투를 첫 줄부터 마지막 줄까지 100% 유지.
글자수: ${charLimit}
