...
반전 선언 1줄 + 이유 2~3줄 + 결론 1~2줄. 근거 1가지만 깊게.`,
  '궁금증형': `[궁금증형 — 2단 구조 필수]
★★ 출력 형식 — 위반 시 전체 무효 ★★
각 글은 반드시 본문과 답글로 구성한다. 본문 뒤에 줄바꿈 후 "[답글]"이라는 구분자를 단독 줄로 넣고, 그 아래에 답글을 쓴다.
예시:
(본문 내용)
[답글]
(답글 내용)

본문: 후킹(극적 상황/결과 먼저) -> 스토리 빌드업(감정 변화) -> 끊기(가장 궁금한 순간에 멈춤). 결론/답을 본문에서 절대 공개 금지.
답글: 본문의 궁금증에 대한 해답/결론/반전 공개 + CTA(질문/댓글 유도). 3~5문장.
첫문장 공식: [극적 상황]+[구체적 숫자/디테일] 또는 [누군가]+[한마디/행동]+[감정 반응]
본문 250~300자 + 답글 100~150자. "[답글]" 구분자가 없으면 출력 실패로 간주한다.`,
};
...
4순위: 바이럴 최적화 — 아래 5요소 중 2개 이상 포함할 것:
  ① 생존 관심사(돈, 직업, 매출 등) ② 댓글 유발(논란, 의견 대립, 질문) ③ 공유 유발(지인에게 보내고 싶은 정보) ④ 구체적 숫자("241만", "월 1000만원", "7가지") ⑤ 손실 회피("안하면 손해", "놓치면 후회")
...
[첫문장 — 전력의 80%]
첫문장이 안 읽히면 뒷 문장이 아무리 좋아도 죽은 글이다. 유형 가이드의 첫문장 공식을 우선 적용한다.
해당하지 않으면 아래 4종 중 택1 (반드시 선택된 말투로 변환해서 쓸 것):
  공감형: 독자의 구체적 고통을 짚는다 (예: 목디스크 병원가도 치료 안 되는 상황)
  욕망형: 구체적 수치로 결과를 제시한다 (예: 월 1000만원 벌어본 사람의 가장 쉬운 방법)
  반전형: 상식을 깨부순다 (예: 스레드 절대 하지 말라는 반전 선언)
  권위+숫자형: 경력/수치로 신뢰를 건다 (예: 12년째 농사짓는 사람의 한마디)
...
단순 정보 나열만으로 끝내지 마라 — 정보형은 항목 설명에 경험 한 줄을 섞고, 그 외 유형은 스토리를 중심에 놓아라. "사실 + 스토리" 조합이 팬을 만든다.
마지막 줄에 댓글을 유발하는 장치를 넣어라: 질문("이거 나만 그래?"), 논란("반박 환영"), 답글 유도("👇 아래에서 계속").`;
