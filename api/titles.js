...
4. 이용자가 키워드를 2개 이상 입력한 경우, 핵심 1개 + 연관 1개 조합으로 문맥 흐름에 맞게 제목을 구성한다. 3개 이상 나열/도배 절대 금지.
5. 제목 길이: 20~28자 (모바일 검색 결과 잘림 방지).
...
Q1. 이 글을 읽을 고객은 정확히 누구인가?
Q2. 그 고객이 원하는 것 또는 피하고 싶은 것은?
...
function parseResponse(raw) {
  ...
  for (let i = 1; i < sections.length; i += 2) {
    ...
    // 제목 30자 초과 시 트리밍 (20~28자 기준에 맞게 여유 포함)
    const trimmed = lines.slice(0, 2).map(title => {
      if (title.length > 30) {
        return title.slice(0, 30).replace(/[,\s]+$/, '');
      }
      return title;
    });
...
핵심 키워드를 제목 첫 10글자 이내에 배치하세요. 키워드가 여러 개인 경우 핵심 1개 + 연관 1개만 자연스럽게 조합하고, 나머지는 후킹 문장에 녹여주세요. 업종 맥락을 반영하세요.`;
