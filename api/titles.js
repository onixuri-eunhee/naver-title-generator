...
4. 키워드는 핵심 1개 + 연관 1개, 최대 2개 조합. 3개 이상은 스팸으로 인식되어 검색 순위 하락. 키워드 나열/도배 절대 금지.
5. 제목 길이: 25~35자 (모바일 홈피드 잘림 방지).
...
function parseResponse(raw) {
  ...
  for (let i = 1; i < sections.length; i += 2) {
    ...
    // 제목 38자 초과 시 트리밍 (25~35자 기준에 맞게 여유 포함)
    const trimmed = lines.slice(0, 2).map(title => {
      if (title.length > 38) {
        return title.slice(0, 38).replace(/[,\s]+$/, '');
      }
      return title;
    });
...
핵심 키워드를 제목 첫 10글자 이내에 배치하고, 연관 키워드 1개만 자연스럽게 조합하세요. 키워드 3개 이상은 절대 금지. 업종 맥락을 반영하세요.`;
