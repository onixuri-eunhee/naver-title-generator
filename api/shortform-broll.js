...
import { extractToken, getClientIp, resolveAdmin, resolveSessionEmail, setCorsHeaders } from './_helpers.js';
import { uploadImageUrlToR2, uploadToR2 } from './_r2.js';
...
async function callGrokImage(prompt, key) {
  ...
  if (!response.ok) {
    throw new Error(`Grok image failed: ${response.status} ${JSON.stringify(data)}`);
  }
...
async function callSeedanceVideo(prompt, key) {
  if (!process.env.SEEDANCE_API_KEY) {
    throw new Error('SEEDANCE_API_KEY is missing');
  }
  ...
  const data = await parseJsonResponse(response);
  if (!response.ok) {
    throw new Error(`Seedance request failed: ${response.status} ${JSON.stringify(data)}`);
  }
...
export default async function handler(req, res) {
  ...
  if (brollSuggestions.length < 3) {
    return res.status(400).json({ error: 'brollSuggestions는 비어 있지 않은 영어 설명 3개가 필요합니다.' });
  }
  if (!scriptContext) {
    return res.status(400).json({ error: 'scriptContext가 필요합니다.' });
  }

  const ip = getClientIp(req);
  const userId = getSafeUserId(email, ip);
