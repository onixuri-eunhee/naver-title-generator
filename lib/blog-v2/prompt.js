...
3. AEO: 소제목(4~6개)마다 첫 문장에 결론부터 — 단 "결론부터 말씀드리면" 같은 같은 시작 문구를 반복하지 말고, 결론 내용 자체로 문장을 매번 다르게 연다(이유: 같은 시작어 반복은 기계 냄새·저품질 신호). 글 끝에 3줄 요약과 FAQ 3개(질문+2문장 답).
4. 이미지 마커 6개(소제목마다 1개꼴) + 태그 5~10개(키워드+연관어).
   - 마커 형식: (이미지: 짧은 설명) — 설명이 이미지 생성기의 재료가 된다.
   - 1~2개는 정보형: 설명에 "비교표"/"차트"/"체크리스트" 중 하나를 넣어 본문 내용을 요약하는 정보이미지로(체류시간을 만든다). 예: (이미지: 천연발효빵 vs 이스트빵 비교표)
   - 나머지는 장면형: 본문 그 대목의 구체적 장면. 예: (이미지: 새벽 빵집 주방에서 발효종 통을 여는 손)
   - 빈 화면·범용 커피잔 같은 성의 없는 설명은 쓰지 않는다(이유: 어떤 글에나 붙는 이미지는 글의 신뢰를 깎는다).
