...
// ─ 디자인 메타 (카테고리별 디자인 토큰 추출용) ─

export const BackgroundTone = z.enum(['dark', 'light', 'mixed']);
export const TextContrast = z.enum(['high', 'medium']);
export const TransitionStyle = z.enum(['cut', 'fade', 'slide', 'mixed']);

export const DesignMetaSchema = z.object({
  titleSizeRatio: z.number().min(0).max(1),
  titlePositionPercent: z.number().min(0).max(100),
  backgroundTone: BackgroundTone,
  avgSceneDurationSec: z.number().min(0.5).max(30),
...
export const VideoAnalysisSchema = z.object({
  ...
  designMeta: DesignMetaSchema.optional(),
});
