...
// lazy evaluation — 매 요청마다 process.env 재평가 (warm instance 환경변수 변경 즉시 반영)
...
export function resolveInternalIdentity(providedKey) {
  ...
  for (const { key, email } of keys) {
    const stored = Buffer.from(key);
    if (provided.length !== stored.length) continue;
...
/**
 * 통합 인증 함수 — Internal Key 우선, Bearer token 폴백.
