    ...
    ? `# 출력 형식 (JSON만, 다른 텍스트 금지)
{
  ...
  "totalDuration": 180,
  "presetUsed": "친근",
  "captionInstagram": "인스타 릴스용 캡션 — 첫 줄 후킹 + 본문 + 해시태그 5~10개 (이모지 금지)",
  "captionYouTube": "유튜브 숏츠용 설명 — 후킹 + 본문 + #Shorts 포함 해시태그 (이모지 금지)"
}`
    : `# 출력 형식 (JSON만, 다른 텍스트 금지)
{
  "scenes": [
...
# 캡션 작성 규칙 — 플랫폼별 2개 동시 생성 (이모지 금지, 페르소나 1인칭 유지)

## captionInstagram (인스타그램 릴스 / 카드뉴스용)
- 길이: 200자 내외 (최대 2,200자 한도 내)
- 첫 줄: 영상 후킹과 연결되는 한 문장 — 인스타는 "더보기" 누르기 전 첫 125자만 노출
- 본문: 영상 핵심 가치 1~2문장
- 해시태그: 5~10개 (벤치마킹 captionPattern.averageHashtagCount 우선, 없으면 5~10)
  - 추천: captionPattern.commonHashtags가 있으면 우선 포함
- CTA: "저장해두세요" / "프로필 @핸들" (인스타는 본문 링크 클릭 불가)

## captionYouTube (유튜브 숏츠용)
- 길이: 300자 내외 (최대 5,000자 한도 내)
- 첫 3줄이 숏츠 피드 설명 영역에 노출됨 — 앞에 핵심 배치
- 본문: 영상 핵심 가치 + 구독/좋아요 유도
- 해시태그: **#Shorts 필수** (피드 편입 조건) + 주제 해시태그 3~5개
- CTA: "구독 부탁드려요" 같은 유튜브 관습 표현`;
}
...
- 총 글자수(공백 제외): 최소 ${Math.round(expectedTotalChars * 0.95)}자, 목표 ${expectedTotalChars}자 (한국어 내레이션 기준). 부족하면 Point 씬 보강.
...
벤치마킹 데이터가 없습니다. 기본 공감 루프 구조 + 후킹 6종 중 토픽에 맞는 유형 선택.`);
  ...
  parts.push(`\n# 출력
위 입력을 바탕으로 JSON 하나만 반환하세요. scenes 개수 ${sceneCount}개 정확히, **captionInstagram + captionYouTube 두 필드 모두 포함**. 이모지 금지. 페르소나 1인칭 유지.${sectionNote}`);
