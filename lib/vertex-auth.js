...
function _parseServiceAccount() {
  // gemini-vertex.js::setupCredentials()와 동일한 env 우선순위 유지
  let raw = process.env.GOOGLE_VERTEX_SERVICE_ACCOUNT_JSON || process.env.GOOGLE_SERVICE_ACCOUNT_JSON || '';
  if (!raw && process.env.GOOGLE_SERVICE_ACCOUNT_KEY) {
    try { raw = Buffer.from(process.env.GOOGLE_SERVICE_ACCOUNT_KEY, 'base64').toString('utf-8'); } catch { raw = ''; }
  }
