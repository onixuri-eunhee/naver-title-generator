...
export const generateScriptFlow = getAi().defineFlow(
  ...
  async (input) => {
    ...
    // 캡션 누락 시 폴백: scenes[0].script + scenes[-1].script + 플랫폼별 기본 해시태그
    const hook = fixed.scenes?.[0]?.script || '';
    const cta = fixed.scenes?.[fixed.scenes.length - 1]?.script || '';
    const legacyCaption = fixed.caption || '';

    let captionInstagram = fixed.captionInstagram || '';
    if (!captionInstagram || captionInstagram.length < 20) {
      captionInstagram =
        legacyCaption ||
        [hook, cta, '#릴스 #숏폼 #인스타'].filter(Boolean).join('\n\n').slice(0, 300);
      validation.warnings.push('captionInstagram 누락 → scenes 기반 폴백 생성');
    }

    let captionYouTube = fixed.captionYouTube || '';
    if (!captionYouTube || captionYouTube.length < 20) {
      captionYouTube =
        [hook, cta, '#Shorts #쇼츠'].filter(Boolean).join('\n\n').slice(0, 400);
      validation.warnings.push('captionYouTube 누락 → scenes 기반 폴백 생성');
    }

    // YouTube는 #Shorts 태그 필수 — 누락 시 강제 추가
