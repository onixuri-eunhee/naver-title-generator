...
export const SHORTFORM_PRESETS = {
  professional: {
    ...
    kinetic: 'static',
    textPosition: 'top',              // 상단: 모션 텍스트(헤드라인)
    cameraMotion: 'static',
    ...
    subtitle: {
      ...
      font: 'Pretendard',
      size: 52,
      position: 'bottom',             // 하단: 자막(내레이션) — 모션과 분리
      bgColor: '#000000',
      bgOpacity: 0.55,
    },
    bgmCategory: 'calm',
  ...
  friendly: {
    ...
    subtitle: {
      ...
      font: 'Noto Sans KR',
      size: 54,
      position: 'bottom',              // 하단: 자막 — 중앙 모션과 분리
      bgColor: '#FFF3E0',
      bgOpacity: 0.9,
    },
  ...
  impact: {
    ...
    colorPreset: 'noir',
    kinetic: 'heavy',
    textPosition: 'center-large',       // 중앙 큰 강조 텍스트
    ...
    subtitle: {
      ...
      font: 'Pretendard',
      size: 48,
      position: 'bottom',                // 하단: 작은 자막 — 중앙 임팩트와 분리
      bgColor: '#D4B87A',
      bgOpacity: 0.95,
    },
    bgmCategory: 'impact',
  ...
  business: {
    ...
    subtitle: {
      ...
      size: 50,
      position: 'bottom',                // 하단: 표준 뉴스 스타일 자막
      bgColor: '#FFFFFF',
      bgOpacity: 1,                      // 단색 블록 — 번지는 효과 방지
      noShadow: true,                    // 자막 렌더러가 text-shadow 스킵
    },
    bgmCategory: 'calm',
...
/**
 * PresetPicker 카드 썸네일용 — colorPreset별 배경/액센트 색 (Remotion presets에서 추출).
 * 사용자가 한눈에 톤 구분 가능하도록.
 */
export const COLOR_PRESET_SWATCHES = {
  midnight:  { bg: '#0b1220', accent: '#ff5f1f', text: '#ffffff' },
  cream:     { bg: '#FDF8F6', accent: '#ff6f61', text: '#1A1A1A' },
  champagne: { bg: '#F5F4F0', accent: '#C8A96E', text: '#2C2620' },
  apricot:   { bg: '#FFFBF5', accent: '#FF9F7A', text: '#3D2820' },
  ocean:     { bg: '#EFF3F8', accent: '#1E3A5F', text: '#0F1E30' },
  noir:      { bg: '#0A0A0A', accent: '#D4B87A', text: '#F5F5F0' },
  sage:      { bg: '#F2F5F0', accent: '#6B8F7B', text: '#2A3628' },
  plum:      { bg: '#1A0F1F', accent: '#C8A87C', text: '#F0E8E0' },
  forest:    { bg: '#0E1812', accent: '#B8A88A', text: '#E8E5DF' },
  rose:      { bg: '#FAF2F4', accent: '#A04A66', text: '#3D1A28' },
};

export function getColorSwatch(colorPresetId) {
  return COLOR_PRESET_SWATCHES[colorPresetId] || COLOR_PRESET_SWATCHES.midnight;
}
...
export function describeKinetic(k) { return KINETIC_LABEL[k] || k; }
export function describeCamera(c) { return CAMERA_LABEL[c] || c; }
export function describeTransition(t) { return TRANSITION_LABEL[t] || t; }
...
export function buildStep6ValueFromPreset(presetId) {
  ...
  return {
    ...
    kinetic: p.kinetic,               // 'static'|'light'|'heavy'|'word-by-word'
    bgmCategory: p.bgmCategory,       // 'calm'|'energetic'|'impact'|'emotional'|'trend'
