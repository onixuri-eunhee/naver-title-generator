...
/**
 * PC 발행 도우미 명령 묶음 생성(사용자가 복사해 실행).
 * @param {{slug:string, scheduleArg:string}[]} items 글 슬러그 + 예약시각
 * @param {string} [publisherPath]
 */
export function toPublisherCommands(items, publisherPath = 'scripts/naver_publisher.py') {
  return items
    .map((it) => `python3 ${publisherPath} "data/blog-drafts/${it.slug}.md" --images-dir "data/blog-drafts/${it.slug}" --schedule "${it.scheduleArg}"`)
    .join('\n');
}
