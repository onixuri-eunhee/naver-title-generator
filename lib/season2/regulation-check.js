...
export function parseVerdict(rawText) {
  ...
  if (!obj || typeof obj !== 'object') {
    ...
    return { verdict: 'fail', violations: [], parseError: true, raw: String(rawText || '').slice(0, 500) };
  }
 ...
 * @returns {{ instruction: string|null, fixable: number }}
 */
export function buildFixInstruction(violations) {
  const withQuote = (violations || []).filter((v) => v && typeof v.quote === 'string' && v.quote.trim());
  if (withQuote.length === 0) return { instruction: null, fixable: 0 };
  const lines = withQuote
    .map((v) => `- "${v.quote}" → ${v.fix_hint || '규정에 맞게 수정'} (${v.reason || ''})`)
    .join('\n');
  return {
    instruction: `아래 문구가 광고 규제에 위반된다. 해당 문구만 규정에 맞게 고쳐라. 나머지 글은 그대로 두고, 전체 원고를 같은 JSON 형식으로 다시 출력하라.
${lines}`,
    fixable: withQuote.length,
  };
}
