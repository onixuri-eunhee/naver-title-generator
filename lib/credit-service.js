 ...
 *
 * 기존 `lib/db.js::chargeCredits` / `refundCredits`는 idempotency가 없고 다른 도구
 * (블로그/카드뉴스/이미지)가 이미 사용 중이라 건드리지 않는다. Phase A-bis의
 * shortform-script/refine 라우트와 향후 전환 경로는 이 파일의 idempotent 버전을 사용한다.
 *
 * 전략 (spec §6.1):
