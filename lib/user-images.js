...
// SSRF 방어용 허용 호스트 — 모듈 로드 시 1회 계산.
// 우리 R2 CDN + R2_PUBLIC_URL 호스트. (data: URL은 네트워크 요청 없어 별도 허용)
const ALLOWED_IMAGE_HOSTS = (() => {
  const hosts = new Set(['cdn.ddukddaktool.co.kr']);
  try {
    const pub = process.env.R2_PUBLIC_URL && new URL(process.env.R2_PUBLIC_URL).hostname;
    if (pub) hosts.add(pub);
  } catch {}
  return hosts;
})();

// SSRF 방어 — 사용자가 직접 주소를 넣는 경계(예: /api/my-images)에서 호출한다.
// 서버가 스스로 만든 신뢰 가능한 URL(예: 이미지 생성 결과)은 이 가드를 거치지 않는다.
// 내부망(169.254.169.254 메타데이터, localhost 등) 접근을 원천 차단한다.
export function assertAllowedImageUrl(sourceUrl) {
  let parsed;
  try {
    parsed = new URL(sourceUrl);
  } catch {
    throw new Error('유효하지 않은 이미지 주소입니다.');
  }

  // data:image/* — 네트워크 요청 없음(클라이언트 canvas 캡처). 허용.
  if (parsed.protocol === 'data:') {
    if (!/^data:image\/(png|jpe?g);/i.test(sourceUrl)) {
  ...
  if (parsed.protocol !== 'https:') {
    throw new Error('허용되지 않은 이미지 출처입니다.');
  }
...
/**
 * 외부 URL(주로 기존 R2 경로)에서 이미지를 가져와 user_images로 등록.
 * - blog-image-pro 결과 "보관함 저장" 버튼
 * - shortform Step 5 AI 이미지 자동 저장
 *
 * 동일 파이프라인(sharp 정규화 + 썸네일 + user-images R2 경로 + DB INSERT)을 재사용.
 * 원본 URL은 images-pro 등 다른 R2 경로라도 상관없음 (재처리 후 user-images 경로에 새로 씀).
 */
export async function registerFromUrl({ email, sourceUrl, filename = null, tag = null, followRedirects = true }) {
  if (!email) throw new Error('email이 필요합니다.');
  if (!sourceUrl || typeof sourceUrl !== 'string') {
    throw new Error('sourceUrl이 필요합니다.');
  }

  // SSRF 가드는 호출 경계(사용자 입력)에서만 적용한다. registerFromUrl은 서버가
  // 생성한 신뢰 가능한 URL(이미지 생성 결과 등)도 받으므로 여기서 호스트를 막지 않는다.
  // 사용자 입력 경로(/api/my-images)는 assertAllowedImageUrl 호출 + followRedirects:false로
  // 호출한다. 리다이렉트 금지는 허용 CDN이 내부주소로 우회하는 SSRF를 막기 위함이며,
  // fal.ai 등 신뢰 가능한 내부 URL은 리다이렉트가 정상이라 기본값은 따라간다(true).
  const res = await fetch(sourceUrl, followRedirects ? undefined : { redirect: 'error' });
  if (!res.ok) {
  ...
  if (buffer.length > MAX_FILE_SIZE) {
    throw new Error('원본 이미지가 5MB를 초과합니다.');
  }

  // 쿼터 체크는 호출 측에서 수행 — 이 함수는 저장만 담당.
  await ensureSchema();
  ...
  const safeTag = tag ? String(tag).trim().slice(0, 50) : null;
  // sourceUrl 마지막 경로를 기본 파일명으로
  const inferredName = (() => {
    try {
      const u = new URL(sourceUrl);
      const last = u.pathname.split('/').pop() || 'saved.jpg';
      return last.length > 255 ? last.slice(-255) : last;
    } catch {
      return 'saved.jpg';
    }
  })();
  const safeFilename = String(filename || inferredName).slice(0, 255);
