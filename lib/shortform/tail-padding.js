...
export function computeTailPadding({ audioRealDurationSec, charEndSec, fps }) {
  ...
  if (
    !Number.isFinite(audioRealDurationSec) ||
    audioRealDurationSec <= 0 ||
    !Number.isFinite(charEndSec) ||
    charEndSec <= 0
  ) {
    return FALLBACK_TAIL_PADDING_FRAMES;
  }
  const driftSec = Math.max(audioRealDurationSec - charEndSec, 0);
  const totalSec = driftSec + SAFETY_BUFFER_SEC;
