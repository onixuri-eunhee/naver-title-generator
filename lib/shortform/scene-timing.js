...
export function deriveSceneDurationsFromCharTimestamps(charAlignment, scenes, opts = {}) {
  const { fps, audioRealDurationSec } = opts;
  ...
  // 각 씬의 start 시간을 charAlignment에서 뽑음
  const sceneStarts = sceneCharRanges.map(({ start: charStart, end: charEnd }) => {
    const firstCharIdx = Math.min(charStart, characters.length - 1);
    if (charStart >= charEnd || firstCharIdx < 0) return null; // 빈 씬
    return starts[firstCharIdx] ?? 0;
  });

  // 각 씬의 end = 다음 씬의 start (gap 포함).
  // 마지막 씬의 end = audioRealDurationSec(있으면) 또는 마지막 char의 end.
  const lastCharIdx = characters.length - 1;
  const lastCharEnd = ends[lastCharIdx] ?? starts[lastCharIdx] ?? 0;
  const audioEnd =
    Number.isFinite(audioRealDurationSec) && audioRealDurationSec > 0
      ? audioRealDurationSec
      : lastCharEnd;

  let durations = sceneStarts.map((startSec, i) => {
    if (startSec == null) return MIN_FIRST_SCENE_FRAMES; // 빈 씬
    // 다음 유효한 씬의 start를 찾음 (빈 씬 건너뜀)
    let nextStartSec = audioEnd;
    for (let j = i + 1; j < sceneStarts.length; j++) {
      if (sceneStarts[j] != null) {
        nextStartSec = sceneStarts[j];
        break;
      }
    }
    const frames = Math.round((nextStartSec - startSec) * fps);
