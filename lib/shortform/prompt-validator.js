...
// - 순수 함수. 사이드 이펙트 없음.
// - errors → 심각(재시도 대상): layoutType 누락/오타, 캡션 누락 또는 중복
// - warnings → 경미(로그만): 데이터 시각화 <2회, onScreenText 길이/누락

import { captionsAreDuplicate, isValidCaption } from './caption-fallback.js';
...
export function validateScriptQuality(parsed) {
  ...
  // 룰 6: 캡션 2종 — 누락 또는 본문 중복 시 재시도 유도 (errors)
  const ig = parsed.captionInstagram;
  const yt = parsed.captionYouTube;
  if (!isValidCaption(ig)) {
    errors.push('caption_instagram_missing');
  }
  if (!isValidCaption(yt)) {
    errors.push('caption_youtube_missing');
  }
  if (isValidCaption(ig) && isValidCaption(yt) && captionsAreDuplicate(ig, yt)) {
    errors.push('caption_duplicate_body');
  }
