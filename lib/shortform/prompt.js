...
- "small-label": 작은 상단 라벨. 카테고리·날짜·출처 등 보조 정보.
- "subtitle-bar": 하단 자막 바. 부연 설명·출처.

[데이터 시각화 계열 — 적극 활용!]
- "counter": 큰 숫자 카운팅 애니메이션. layoutProps: { value: 숫자, suffix: "단위", label: "설명" }
- "progress-bar": 프로그레스 바. layoutProps: { value: 87, label: "달성률" }
- "bar-chart": 막대 그래프. layoutProps: { bars: [{label:"A", value:30}, {label:"B", value:70, highlight:true}] }
- "pie-chart": 도넛 차트. layoutProps: { slices: [{label:"마케팅", value:40}, {label:"상담", value:35}, {label:"관리", value:25}], centerLabel: "시간배분", centerValue: "100%" }
- "venn-diagram": 벤 다이어그램(2~3개 원). layoutProps: { circles: [{label:"기획"}, {label:"실행"}], intersectionLabel: "성장" }
- "comparison-chart": 기능 비교표. layoutProps: { leftLabel: "기존", rightLabel: "AI 활용", rows: [{feature:"소요시간", left:"3시간", right:"5분"}, {feature:"자동화", left:false, right:true}], highlightRight: true }
- "flow-diagram": 단계/프로세스 흐름도. layoutProps: { steps: [{label:"1", title:"조사"}, {label:"2", title:"기획"}, {label:"3", title:"실행"}], direction: "vertical" }
- "network": 노드 연결망. 관계도/시간배분에 사용. layoutProps: { nodes: [{x:270, y:480}, {x:100, y:200}, ...], edges: [[0,1], [0,2], ...], width: 540, height: 960 }
- "strikethrough": 취소선 강조. 기존 믿음을 부정할 때 사용. layoutProps: { strikeWord: "취소할단어" }
- "number-slam": 숫자/텍스트 바운스 등장. 임팩트 있는 숫자 공개에 사용. layoutProps: { subtitle: "보조설명" }

[레거시 — 텍스트 비교 (단순한 비교에 사용)]
- "comparison": 좌우 비교표. layoutProps: { leftTitle: "일반", rightTitle: "뚝딱툴", rows: [{feature:"시간", left:"3시간", right:"5분"}] }

규칙:
- 첫 씬(hook)은 "big-impact-text" 권장.
- 숫자/통계 언급 시 반드시 "counter" 또는 "bar-chart" 사용.
- 비율/구성 언급 시 "pie-chart" 사용.
- 개념 관계/교집합 언급 시 "venn-diagram" 사용.
- 단계/프로세스 설명 시 "flow-diagram" 사용.
- 기능 비교 시 "comparison-chart" 사용 (boolean 지원, ✓/✕ 표시).
- 마지막 씬(CTA)은 "big-impact-text" 권장.
- 모든 씬에 layoutType 지정 필수 (빠뜨리지 마세요).
- 대본 전체에서 데이터 시각화 계열을 최소 2~3회 이상 사용하세요. 텍스트만 나열하면 안 됩니다.
