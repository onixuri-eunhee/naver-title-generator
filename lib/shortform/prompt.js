...
function buildLayoutTypeBlock({ visualStyle }) {
  ...
  return `[비주얼 레이아웃 — 텍스트형 모드 (9:16 세로 숏폼)]
각 씬에 layoutType 필드를 반드시 포함하세요. 이미지 없이 텍스트/데이터 시각화만으로 구성합니다.
화면은 세로(1080×1920) 9:16 비율입니다. 모든 레이아웃은 세로 화면에 최적화해야 합니다.

⚠ 절대 금지: 이모지, 이모티콘 사용 금지. icon 필드에 이모지를 넣지 마세요.

사용 가능한 layoutType:

[텍스트 계열]
- "big-impact-text": 대형 텍스트. 후킹·CTA·핵심 메시지.
- "emphasis-box": 강조 박스. layoutProps: { variant: "check"|"warning"|"info" }
- "bullet-list": 불릿 리스트. layoutProps: { items: ["항목1", "항목2", ...] }
- "vertical-bar": 세로 바 + 텍스트. 구분선 강조.
- "strikethrough": 취소선 강조. layoutProps: { strikeWord: "취소할단어" }
- "number-slam": 숫자 바운스 등장. layoutProps: { subtitle: "보조설명" }

[데이터 시각화 계열 — 적극 활용!]
- "counter": 숫자 카운팅. layoutProps: { value: 숫자, suffix: "단위", label: "설명" }
- "progress-bar": 프로그레스 바. layoutProps: { value: 87, label: "달성률" }
- "bar-chart": 막대 그래프(2~4개 막대 권장). layoutProps: { bars: [{label:"A", value:30}, {label:"B", value:70, highlight:true}] }
- "pie-chart": 도넛 차트. layoutProps: { slices: [{label:"마케팅", value:40}, {label:"상담", value:35}], centerLabel: "시간배분", centerValue: "100%" }
- "comparison-chart": 비교표. layoutProps: { leftLabel: "기존", rightLabel: "AI 활용", rows: [{feature:"소요시간", left:"3시간", right:"5분"}, {feature:"자동화", left:false, right:true}], highlightRight: true }
- "flow-diagram": 단계 흐름도(반드시 세로 방향). layoutProps: { steps: [{label:"1", title:"조사"}, {label:"2", title:"기획"}, {label:"3", title:"실행"}] }

규칙:
- 첫 씬(hook)은 "big-impact-text".
- 마지막 씬(CTA)은 "big-impact-text".
- 숫자/통계 → "counter" 또는 "bar-chart".
- 비교 → "comparison-chart".
- 단계/프로세스 → "flow-diagram" (direction 지정하지 마세요, 기본값이 세로).
- 대본 전체에서 시각화 계열을 2~3회 이상 사용. 텍스트만 나열하면 안 됩니다.
- 모든 씬에 layoutType 필수.
- layoutProps에 이모지·이모티콘 절대 금지.

[예시 1 — 매출 관리 주제]
scenes: [
  {section:"hook", layoutType:"big-impact-text", script:"매출 1,000만 원, 세금은 얼마?"},
  {section:"point", layoutType:"counter", layoutProps:{value:1000, suffix:"만원", label:"연매출"}, script:"연 매출 천만 원 기준"},
  {section:"point", layoutType:"bar-chart", layoutProps:{bars:[{label:"소득세",value:35,highlight:true},{label:"부가세",value:45},{label:"지방세",value:20}]}, script:"세금 종류별 비중"},
  {section:"point", layoutType:"comparison-chart", layoutProps:{leftLabel:"직접 관리",rightLabel:"세무사 위탁",rows:[{feature:"비용",left:"0원",right:"월 11만원"},{feature:"실수 위험",left:true,right:false}],highlightRight:true}, script:"직접 vs 위탁 비교"},
  {section:"point", layoutType:"emphasis-box", layoutProps:{variant:"check"}, script:"매출 4,800만 원 이하면 간이과세 가능"},
  {section:"cta", layoutType:"big-impact-text", script:"지금 바로 확인해보세요"}
]

[예시 2 — 1인 사업가 AI 활용]
scenes: [
  {section:"hook", layoutType:"big-impact-text", script:"AI 안 쓰는 사장님, 혼자 야근하고 계시죠?"},
  {section:"point", layoutType:"flow-diagram", layoutProps:{steps:[{label:"1",title:"고객 문의 정리"},{label:"2",title:"견적서 작성"},{label:"3",title:"SNS 콘텐츠"}]}, script:"하루 업무 3단계"},
  {section:"point", layoutType:"counter", layoutProps:{value:4.5, suffix:"시간", label:"하루 절약"}, script:"AI로 매일 4.5시간 절약"},
  {section:"point", layoutType:"bullet-list", layoutProps:{items:["ChatGPT로 고객 응대 자동화","Canva로 SNS 이미지 생성","뚝딱툴로 숏폼 영상 제작"]}, script:"실전 AI 도구 3가지"},
  {section:"cta", layoutType:"big-impact-text", script:"오늘부터 시작하세요"}
]`;
}
