...
export function buildUserPrompt({
  ...
  // Sprint 1 (4/22) — 코퍼스 few-shot 주입 우선, 없으면 기존 patterns, 없으면 skip.
  // corpusExamples 는 resolveBenchmark(source='corpus') 가 제공.
  // patterns 는 source='haiku' 혹은 legacy fallback 일 때.
  let benchmarkSection = '';
  const corpusExamples = Array.isArray(benchmark?.corpusExamples) ? benchmark.corpusExamples : [];

  if (corpusExamples.length > 0) {
    const sampleBlocks = corpusExamples.map((s, i) => {
      const im = s.identityMirror || {};
      const stats = s.stats || {};
      const views = (stats.views || 0).toLocaleString();
      const subs = (stats.subs || 0).toLocaleString();
      const ratio = Number(stats.ratio || 0).toFixed(1);
      const actions = Array.isArray(im.actions) ? im.actions.filter(Boolean) : [];
      const consequences = Array.isArray(im.consequences) ? im.consequences.filter(Boolean) : [];
      const sceneLines = (s.sceneSampleBrief || []).map(
        (sc) => `    · [${sc.role}] ${sc.script}${sc.narrationTone === 'analytical' ? '  ⚠️분석톤' : ''}`,
      ).join('\n');

      return `### 샘플 ${i + 1} — "${s.title || ''}"
   조회 ${views} / 구독 ${subs} / 비율 ${ratio}x
   첫 문장: "${im.firstSentence || ''}"
   6-beat: ${im.timeAdverb ? `[${im.timeAdverb}] ` : ''}${actions.length ? `행동=${JSON.stringify(actions)} → ` : ''}${consequences.length ? `결과=${JSON.stringify(consequences)} → ` : ''}심리="${im.emotion || ''}"
   hookType=${im.hookType || '?'} · emotionTrigger=${im.emotionTrigger || '?'} · 어미=${im.firstSentenceEnding || '?'}
${im.definedEmotion ? `   정의된 감정: "${im.definedEmotion}"\n` : ''}   씬 흐름:
${sceneLines}
   CTA: ${s.cta?.type || 'none'} · "${s.cta?.phrase || ''}"`;
    }).join('\n\n');

    benchmarkSection = `

[★★★ 이 주제 실제 바이럴 샘플 ${corpusExamples.length}개 — 공식 복제 ★★★]

${sampleBlocks}

[★ 이 샘플들로부터 배울 것 (사용자 3대 원칙)]

원칙 1 — Identity Mirror 3단 구체화 (scene[0] 필수)
  첫 문장에 [시간부사] + [행동 2개] + [결과 2개] + [심리 1개] 가 모두 들어가야 한다.
  한 단만 있으면 AI 일반화. 세 단 다 있어야 "내 얘기".
  예: "아직도 장부 옮겨적고 엑셀로 고객관리하시다 데이터 엉망진창 관리 자체가 안 되어 손 놓고 계시죠"

원칙 2 — 단정·도발 어미 (AI 틱 금지)
  금지: ~때문입니다 / ~로 인해 / ~의 결과입니다 / ~에 기인
  선호: ~거예요 / ~잖아요 / ~거든요 / 그냥 ~한 거예요
  분석가 아닌 현장 목격자로 말한다.

원칙 3 — Polarity 선 긋기 (애매 금지)
  금지: ~일 수 있어요 / 경우에 따라 / 한편 / 물론 ~도 있지만 / ~하기 나름
  선호: 단정·도발로 찬/반 양극 프레임. 시청자의 미정의 감정에 이름을 붙인다.
  예: "사실 당신은 포기하고 싶었던 거예요"

[예외 허용] scene[0] 이 스토리텔링 hook 인 경우 (ex. "기억에 남는 분이 있나요?" 같이 답 안 주고 궁금증 유도) Identity Mirror 규칙 완화 가능 — 대신 뒷 씬에서 강한 공감대 형성.

[★ 위반 금지 — 이 규칙은 [절대 규칙]보다 우선한다]
- 위 샘플의 패턴과 공식을 무시하고 일반화된 대본 작성 시 실패.
- 샘플이 없는 필드(샘플별로 비어있는 값)는 새로 채우되 위 3원칙은 항상 지킨다.`;
  } else if (benchmark && benchmark.patterns && !benchmark.fallback) {
    // 코퍼스 없을 때 기존 Haiku patterns fallback
