...
async function ensureSchema() {
  ...
  _schemaReady = (async () => {
    ...
    await sql`CREATE TABLE IF NOT EXISTS shortform_projects (
      ...
      content_type VARCHAR(20) DEFAULT 'shortform',
      created_at TIMESTAMPTZ DEFAULT NOW(),
      updated_at TIMESTAMPTZ DEFAULT NOW(),
      published_at TIMESTAMPTZ
    )`;
    // v2.1: 기존 테이블에 content_type 컬럼 추가 (마이그레이션)
    await sql`ALTER TABLE shortform_projects
      ADD COLUMN IF NOT EXISTS content_type VARCHAR(20) DEFAULT 'shortform'`;
...
export async function createDraft(email, initial = {}) {
  ...
  const rows = await sql`INSERT INTO shortform_projects
    (user_email, status, current_step, title,
     blog_text, keywords, user_experience, persona, tone, duration_sec, content_type)
    VALUES (${email}, ${'draft'}, ${Number(initial.current_step) || 1},
            ...
            ${initial.content_type || 'shortform'})
    RETURNING *`;
  return rows[0];
}
...
export async function duplicateProject(email, id) {
  ...
  const rows = await sql`INSERT INTO shortform_projects
    ...
    VALUES (${email}, ${'draft'}, ${1},
            ...
            ${original.content_type || 'shortform'},
