...
export function validateScriptOutput(parsed, opts = {}) {
  ...
  // caption 검증 — 플랫폼별 2개 필드 (captionInstagram / captionYouTube).
  // 기존 caption 필드(레거시)도 그대로 유지해 script-flow.js의 fallback이 활용.
  let fixedCaption = parsed.caption || '';
  let fixedCaptionInstagram = parsed.captionInstagram || '';
  let fixedCaptionYouTube = parsed.captionYouTube || '';

  function validateOneCaption(text, label) {
    const emojis = detectEmojis(text);
    if (emojis.length > 0) {
      errors.push(`${label}에 이모지 ${emojis.length}개 검출: ${emojis.map((e) => e.char).join('')}`);
      text = stripEmojis(text);
    }
    const generic = detectGenericPhrases(text);
    if (generic.length > 0) {
      warnings.push(`${label} 일반론 표현: ${generic.map((g) => g.match).join(', ')}`);
    }
    return text;
  }

  if (fixedCaption) fixedCaption = validateOneCaption(fixedCaption, 'caption');
  if (fixedCaptionInstagram) fixedCaptionInstagram = validateOneCaption(fixedCaptionInstagram, 'captionInstagram');
  if (fixedCaptionYouTube) fixedCaptionYouTube = validateOneCaption(fixedCaptionYouTube, 'captionYouTube');

  // 총 글자수 (대략적)
  const totalChars = fixedScenes.reduce((sum, s) => sum + (s.script || '').replace(/\s+/g, '').length, 0);
  if (durationSec) {
    const expected = durationSec * 5;
    // 롱폼(>=180s)은 Claude가 상한에서 다소 짧게 나올 여지가 있어 tolerance 완화 (50%)
    const toleranceRatio = durationSec >= 180 ? 0.5 : 0.3;
    const tolerance = Math.ceil(expected * toleranceRatio);
  ...
  return {
    ...
    autoFixed: {
      ...
      captionInstagram: fixedCaptionInstagram,
      captionYouTube: fixedCaptionYouTube,
    },
  };
}
